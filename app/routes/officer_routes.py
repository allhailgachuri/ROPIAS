...
from database.models import db, User, QueryLog, AlertLog
...
@officer_bp.route("/alerts")
@login_required
@admin_required
def alert_log():
    """Full alert history — all WhatsApp and SMS messages sent."""
...
@officer_bp.route("/system")
@login_required
@admin_required
def system_status():
    """System health — API status, cache stats, alert credits."""
    return render_template("officer/system.html", user=current_user)
